...
def process_files_ddi(files, triple_file, saved_relation2id=None, keeptrainone = False):
    ...
    for file_type, file_path in files.items():# {"train":path,"valid":path,"test":path}
        data = []
        # with open(file_path) as f:
        #     file_data = [line.split() for line in f.read().split('\n')[:-1]]
        ...
        for triplet in file_data:
            #print(triplet)
            triplet[0], triplet[1], triplet[2] = int(triplet[0]), int(triplet[1]), int(triplet[2]) # drug1 drug2 relation   10  1  0
            if triplet[0] not in entity2id:
            ...
            if not saved_relation2id and triplet[2] not in relation2id:
                if keeptrainone: 
                    triplet[2] = 0 
                    relation2id[triplet[2]] = 0 #{0:0}
                    rel = 1
                else:
                    relation2id[triplet[2]] = triplet[2] # {1:1,0:0,7:7,...} 
                    rel += 1 
        ...
        triplets[file_type] = np.array(data) # {"train":[[10,1,0],[],[],...],"valid":[[],[],[],...],......}
    #print(rel)
    ...
    for (h, t, r) in triplet_kg:
        ...
        kg_triple.append([h, t, r]) # [[h,t,r],[h,t,r],...] 
    ...
    #for i in range(len(relation2id)):
    for i in range(rel):
        idx = np.argwhere(triplets['total'][:, 2] == i) 
        # adj_list.append(csc_matrix((np.ones(len(idx), dtype=np.uint8), (triplets['train'][:, 0][idx].squeeze(1), triplets['train'][:, 1][idx].squeeze(1))), shape=(len(entity2id), len(entity2id))))
        adj_list.append(csc_matrix((np.ones(len(idx), dtype=np.uint8), (triplets['total'][:, 0][idx].squeeze(1), triplets['total'][:, 1][idx].squeeze(1))), shape=(len(entity2id), len(entity2id))))
